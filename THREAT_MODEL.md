...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Этапы 2a/2b завершены в обеих средах; owner/migrator завершён в Preview 2026-08-13.** Preview direct endpoint прошёл два apply, target guard, no-op Prisma, ownership-probe и post-audit; GitHub Preview secret больше не содержит owner credential. Neon-специфичный двойной grant закреплён fail-closed и проверяется локально через non-superuser `CREATEROLE` admin. Следующие шаги — реальный Preview workflow gate после публикации, отдельный Production migration scope, Production backup scope, scoped context и tenant-RLS |
