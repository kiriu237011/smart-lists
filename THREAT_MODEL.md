...
Последняя сверка с кодом: 2026-08-11 (автоматическая синхронизация Preview OAuth proxy)
...
Приоритет 1.3 закрыт целиком: 2026-08-10 — секретов в системе не осталось
Приоритеты 2.6–2.8 закрыты: 2026-08-09 — non-root, `openapi_url`, Force TLS
Приоритет 2.2 закрыт: 2026-08-10 — versioning на бакетах вложений
Приоритет 2.3 закрыт: 2026-08-10 — whitelist сверяется при каждом чтении сессии
Приоритет 2.1 закрыт: 2026-08-10 — суточный бюджет мутаций и потолки на сущности
Приоритет 2.4 закрыт: 2026-08-10 — флаг `aiEnabled` и строка о передаче данных
Приоритет 2.5 переведён в принятые риски: 2026-08-10 — мера не закрывала свой сценарий

**Ключевой сдвиг 2026-08-10.** Раздел «Активы» открывался двумя строками
секретов; теперь их ноль. Оба упразднены, а не перепрятаны: shared secret
заменён Google ID-токеном, ключ Anthropic — workload identity federation.
Это меняет не оценку рисков, а сам их состав — угрозы вида «секрет прочитан,
секрет утёк, секрет надо ротировать» не смягчены, а лишены предмета.
...
Получился бы контроль, который выглядит как защита и ею не является, — ровно тот класс ошибки, который этот документ у себя уже дважды находил («non-root контейнер», «отсутствие ретраев»). Поэтому мера не сделана наполовину, а отложена целиком: настоящий вариант требует унести миграции из сборки Vercel в отдельный шаг CI, где владелец живёт только в GitHub Secrets. Это изменение процесса выкладки со своими вопросами — порядок «миграция раньше трафика» и поведение при «миграция прошла, деплой упал», — и оно не оправдано при одном администраторе.
...
| A31 | Владельческая строка подключения доступна работающему приложению | Пока это так, любое разделение ролей Postgres декоративно: рядом лежит переменная с DDL | ❌ **ложно как защита, верно как факт**: `DIRECT_URL` задан в окружении Vercel, потому что `build:deploy` применяет миграции внутри сборки. Сломается — то есть станет неверным в хорошем смысле — при переносе миграций в отдельный шаг CI |
...
|---|---|---|
| ~~**A. Нет rate limit на обычные Server Actions**~~ | D×3 | **закрыт 08-10** суточным бюджетом мутаций: он один ограничивает все три — storage, compute и квоту Pusher, потому что каждая мутация это и запись, и запрос, и событие |
| ~~**B. Нет versioning на бакетах**~~ | T×1, D×1 | **закрыт целиком**: затирание бэкапа 08-09, необратимое удаление вложений 08-10 |
...
| ~~2.5~~ | ~~Отдельная роль Postgres без прав DDL~~ | **Accept** | ⏸ **Отложено 2026-08-10, риск принят.** Разбор перед реализацией показал, что мера в предложенном виде не закрывает сценарий, ради которого задумана: миграции применяет `build:deploy` из сборки Vercel под `DIRECT_URL`, и обе строки подключения лежат в одном окружении. Роль без DDL у runtime не мешает злоумышленнику взять соседнюю переменную с владельцем. Настоящий вариант требует унести миграции из сборки в отдельный шаг CI — это изменение процесса выкладки, а не настройка прав. См. «Принятые риски» |
...
- Перенос `prisma migrate deploy` из сборки Vercel в отдельный шаг CI. Сам по себе не защита, но именно он делает осмысленной отдельную роль Postgres без DDL (2.5) — сейчас та бессмысленна, потому что владельческая строка лежит рядом с runtime-строкой
...
- ~~Чистка старых строк счётчиков~~ — сделано 2026-08-10 вместе с 2.1: ленивая уборка, 30 суток
...
| Приложение ходит в БД под ролью-владельцем с правами DDL | Разделение ролей само по себе ничего не даёт, пока миграции идут из сборки Vercel: владельческая строка лежит в том же окружении. Полумера создала бы видимость защиты. Ущерб ограничен тем, что дампы защищены versioning и проверены на восстановимость, а PITR Neon покрывает шесть часов | Появление второго человека с доступом к Vercel; публикация приложения за пределы whitelist; перенос миграций в отдельный шаг CI — тогда роль без DDL становится осмысленной и делается сразу |
