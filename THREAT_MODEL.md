...
Приоритет 1.1 закрыт: 2026-08-09 — AI-сервис больше не вызывается анонимно
Приоритет 1.2 закрыт: 2026-08-09 — versioning на бэкап-бакете
...
| TB4 | Next.js → FastAPI | IAM Cloud Run (ID-токен в `X-Serverless-Authorization`), затем shared Bearer secret, `hmac.compare_digest`, потолок тела 100 KB, `5/minute` |
...
| Буква | Статус | Суть |
|---|---|---|
| T | partial | TOCTOU на presigned POST; versioning включён на бэкап-бакете 2026-08-09, на бакетах вложений ещё нет |
...
| I | closed / partial | содержимое и секреты не логируются, generic 5xx; `/openapi.json` закрыт вместе с сервисом, но в коде не погашен |
...
**Закрыто 2026-08-09.** `allUsers` снят, право `run.invoker` есть только у `vercel-insights-invoker`. Vercel предъявляет Google ID-токен в `X-Serverless-Authorization`, Cloud Run проверяет его сам и до кода сервиса; анонимный запрос получает 403 на всех путях, включая `/health` и `/openapi.json`. Токен выпускается через Workload Identity Federation, долгоживущих ключей на пути нет. Shared secret сохранён вторым слоем.
...
3. **Вес статического секрета упал, но проблема ротации осталась.** Раньше он был единственным барьером перед платным путём; теперь перед ним стоит IAM. Ротацию это упрощает, а не отменяет — см. A8 и пункт 1.3.
...
С 2026-08-09 снаружи он недоступен — закрылся вместе с сервисом, вместе со всем остальным. **Но закрылся не там, где сломан.** В коде `openapi_url` по-прежнему дефолтный, и схема вернётся в публичный доступ в ту же секунду, когда сервис по любой причине снова окажется открыт. Это ровно тот случай, ради которого документ различает «угроза не реализуема» и «контроль есть»: сейчас верно первое. Починка остаётся в плане (2.7) и стоит одной строки: `openapi_url="/openapi.json" if settings.debug else None`.
...
| A9 | Versioning включён на бэкап-бакете | Затирание единственного пути восстановления | ✅ **закрыто 08-09**: включён и проверен поведением — перезапись занятого ключа сохранила прежнюю версию. На бакетах вложений всё ещё выключен (2.2) |
...
| A18 | Контракт сервиса не опубликован | Разведка перестаёт что-либо стоить атакующему | ⚠️ снаружи закрыт вместе с сервисом (08-09), но в коде `openapi_url` не погашен — вернётся, если сервис снова откроют |
| A20 | Только Production обращается к AI-сервису | Секрет и доступ к общему сервису расходятся по средам | ✅ провайдер OIDC пускает единственную среду `production`; в Preview не заданы `INSIGHTS_SERVICE_*` |
...
| **B. Нет versioning на бакетах** | T×1, D×1 | ~~затирание бэкапа~~ (закрыто 08-09), необратимое удаление вложений (2.2) |
...
| 1.3 | Перенести `SERVICE_SECRET` и `ANTHROPIC_API_KEY` в Secret Manager; заодно сузить `github-deployer` с `run.admin` до `run.developer` | Mitigate | Подтверждено: plain env. Это разрывает связь «право деплоить ⇒ право прочитать секреты» и делает ротацию возможной без уборки старых ревизий. После закрытия 1.1 — крупнейший оставшийся пункт по секретам |
