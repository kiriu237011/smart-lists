...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Release, runtime и operational owner/migrator/backup этапы завершены 2026-08-13; foundation, `spaces`, server read-path, user quota, space mutations, DB-фазы AI insights и attachments реализованы 2026-08-14.** Прямой allowlist сократился с 13 до 6; DB-тесты доказывают ownership/share-space semantics, разделение reads, конкурентную атомарность обоих бюджетов, post-commit внешние эффекты и fail-closed AI/attachment потоки. S3/AI/Pusher не удерживают DB-транзакции; attachment realtime больше не читает tenant-таблицы в `after()`. Для глобальной attachment-квоты и cross-space stale-cleanup до enforcement нужен узкий DB-helper — обычная space-policy изменила бы семантику. Остальные tenant-мутации и специальные потоки ещё не перенесены, RLS выключен, поэтому фактическая DB-изоляция строк пока не завершена. Следующие шаги — перенос оставшихся потоков, helper/policies и поэтапное включение tenant-RLS |
