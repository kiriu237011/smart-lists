...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Этап 2a завершён; этап 2b подготовлен локально 2026-08-13.** Target guards и no-op миграции прошли для Production и Preview, production promotion дождался migration job того же SHA, Vercel build больше не применяет миграции, а `DIRECT_URL` удалён из обеих сред Vercel. Точная runtime DML-матрица и fail-closed role tooling проверены на Docker PostgreSQL: 213 integration-тестов прошли под ролью без DDL/BYPASS/ownership. Preview/Production credential ещё не переключались, поэтому фактические облачные DB-права и прикладные контроли пока не изменились |
