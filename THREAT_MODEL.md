...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Release, runtime и operational owner/migrator/backup этапы завершены 2026-08-13; foundation и первая consumer-группа `spaces` реализованы 2026-08-14.** Прямой allowlist сократился с 13 до 12; ownership/share-space semantics доказаны DB-тестом. Остальные tenant-потребители ещё не перенесены и RLS выключен, поэтому фактическая DB-изоляция строк пока не изменилась. Следующие шаги — перенос оставшихся потоков, policies и поэтапное включение tenant-RLS |
