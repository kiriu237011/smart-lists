...
**Последняя сверка с кодом:** 2026-08-12

**Последняя проверка живой инфраструктуры:** 2026-08-10
...
2. **Postgres least privilege и RLS:** архитектурный проект согласован
   2026-08-12. Release-контур этапа 2a слит и включён, но первый main target
   guard безопасно отказал из-за пустых Environment secrets на reusable-
   границе; миграция не запускалась. `build:deploy` и фактические права прежние.
...
Оба feature flags включены 2026-08-12, PR №59 слит в `main`. Все четыре
предварительных gate прошли, после чего target guard получил пустые
`DIRECT_URL` и `EXPECTED_DATABASE_HOST` внутри reusable workflow и остановил
job до подключения; миграция не запускалась. Исправление встраивает production
job в `ci.yml`, не меняя порядок gates или объём секретов, но ещё не проверено
main-прогоном. Vercel всё ещё выполняет `build:deploy`. Факт настройки
Deployment Check вне репозитория не подтверждён; до подтверждения A32 удалять
миграцию из Vercel build нельзя.
...
`ENABLE_PREVIEW_MIGRATION` имеют значение `true`. Первый main run
`31579464717` подтвердил fail-closed поведение: оба секрета оказались пустыми
на reusable-границе, guard завершился ошибкой `DIRECT_URL не задан`, а шаг
миграции был пропущен. Значения и соответствие Neon-ветке ещё не проверены.
...
| A31 | Владельческая строка подключения доступна работающему приложению | Пока это так, любое разделение ролей Postgres декоративно: рядом лежит переменная с DDL | ❌ **ложно как защита, верно как факт**: `DIRECT_URL` задан в окружении Vercel, потому что `build:deploy` применяет миграции внутри сборки. Этап 2a только подготовлен; факт изменится после проверенного cutover |
| A32 | Vercel удерживает production alias до успешной DB migration job того же SHA | Без gate новый код может стать доступен со старой схемой, даже если миграции вынесены в GitHub | ❌ **ещё не контроль**: Deployment Check не подтверждён; первый включённый main target guard безопасно отказал до миграции из-за пустых secrets на reusable-границе. До исправления и успешного no-op прогона `build:deploy` сохраняется |
...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Этап 2a включён 2026-08-12, но ещё не доказан.** Первый main target guard fail-closed остановился до миграции: reusable workflow не получил Environment secrets. Исправление в процессе; `build:deploy` сохраняется до успешного no-op прогона и ручного go/no-go по A32. Текущий риск и прикладные контроли пока не изменились |
