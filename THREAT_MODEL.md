...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Release, runtime, operational roles/backup и scoped ordinary tenant data plane завершены; attachment helper локально готов 2026-08-14.** Глобальная quota и cross-space stale-cleanup проходят через две fail-closed owner-функции: DB-generated token, fixed `search_path`, повторная user/space/list проверка, минимальный `{token,key}`, точечный runtime EXECUTE и отсутствие PUBLIC/backup EXECUTE. `CLEANUP_PENDING` сохраняет metadata до результата S3; failure возвращает только свои токены, success/finalize безопасно повторяется. Role lifecycle, cross-space, чужой token и rollback проверены на реальной БД. Live-среды и RLS этим commit не меняются, runtime пока сохраняет table-wide DML. Следующие шаги — policies без enforcement, отрицательные Alice/Bob тесты и поэтапное включение tenant-RLS |
...
|---|---|---|
| 1 | Подготовить tenant-RLS policies без немедленного enforcement | Узкий attachment helper локально готов; реальный DB-контроль появится только после проверки и включения policies |
