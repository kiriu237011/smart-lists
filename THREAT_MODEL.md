...
подписанная attestation ещё не реализована; 2026-08-30 — этап 2 завершён:
production run `33312038124` проверил BuildKit SLSA v1 `mode=max` exact digest
и развернул тот же digest в Cloud Run
...
| A29 | Каждое действие, меняющее данные, списывает суточный бюджет | Забытый вызов в новом Action означает путь роста в обход лимита — незаметный, потому что всё работает | ⚠️ **частично; уточнено 2026-09-01.** `usage.int.test.ts` требует отказа при исчерпанном бюджете от 25 действий, но список ведётся руками, поэтому полноты он не доказывает: новый Action в массив не попадёт, и прогон останется зелёным. Это уже реализовалось — `requestUpload`, `confirmUpload` и `deleteAttachment` списывают бюджет в коде, но в тесте бюджета отсутствуют, а `attachments.int.test.ts` его не упоминает вовсе. Прежняя редакция строки утверждала полноту прямо («новое действие без бюджета красит прогон») — тот же дефект формы, что дважды был у A51, и найден он тем же способом: проверкой утверждения, а не чтением документа. Закрывается сверкой массива с фактическим набором экспортируемых Actions | R | 2026-09-01 |
...
| A13 | Обычные CI checks и dependency install не получают `DIRECT_URL` | Расширение поверхности supply chain | ✅ реальный credential доступен только guarded migration steps и backup workflow; обычные checks работают без него, placeholder тоже удалён | T | каждый CI |
...
Распределение на 2026-09-01: **T — 31, M — 23, — — 9, R — 8, T\* — 2**. Практический вывод в том, что почти половина реестра (31 строка: M и R) держится на доказательстве, которое устаревает без единого сигнала.
...
| 4 | Проверить production и эксплуатационный контур | ✅ Выполнено 08-31. Run `33391706750` для serving `sha256:498cd37a…5f1a70` доказал его членство в подписанном OCI index `sha256:e727018e…3cd9701` и прошёл recurring provenance gate. Live-негативные проверки отклонили подмену digest, signer и отсутствие attestation. Scanner остался read-only. Следующий независимый CVE gate ожидаемо `BLOCKED`: exact VEX старого digest не переносится |
