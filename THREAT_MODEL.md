...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Release, runtime и operational owner/migrator/backup этапы завершены 2026-08-13; foundation scoped context реализован 2026-08-14.** Wrapper и fail-closed тесты готовы локально, но production-потребители ещё не перенесены и RLS выключен, поэтому фактическая изоляция строк пока не изменилась. Следующие шаги — перенос tenant-потоков, policies и поэтапное включение tenant-RLS |
