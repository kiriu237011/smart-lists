...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Этап 2a завершён; этап 2b дошёл до Preview target 2026-08-13.** Target guards и no-op миграции прошли для Production и Preview, production promotion дождался migration job того же SHA, Vercel build больше не применяет миграции, а `DIRECT_URL` удалён из обеих сред Vercel. Точная runtime DML-матрица и fail-closed role tooling проверены на Docker PostgreSQL: 213 integration-тестов прошли под ролью без DDL/BYPASS/ownership. Read-only audit подтвердил текущий owner-риск; затем `smartlists_runtime` создана и post-check пройден только в `dev`, Production не менялся. Vercel Preview credential ещё не переключён, поэтому фактические права приложения пока не изменились |
...
| Приложение пока ходит в БД под ролью-владельцем с правами DDL | Риск временно сохраняется: release cutover убрал соседний owner credential, но ещё не сузил сам `DATABASE_URL`. Дампы защищены versioning и проверены на восстановимость, PITR Neon покрывает шесть часов | **Mitigation in progress с 2026-08-12:** release migration/cutover, матрица, rollback, restricted-role tests и read-only cloud audit завершены; ограниченная роль создана и проверена в `dev`. Но Vercel Preview всё ещё использует owner, поэтому следующий отдельный шаг — Preview credential cutover, затем Production go/no-go, scoped context и tenant-RLS. Немедленный пересмотр также при втором человеке с доступом к Vercel или снятии whitelist |
