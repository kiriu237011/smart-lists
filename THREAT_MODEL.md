...
| A80 | Приложение обращается к S3 по короткоживущим credentials, а не по статическому ключу | Ключ IAM в переменных Vercel — единственный оставшийся в проде секрет, который не истекает сам. Его нельзя ни проверить на утечку, ни отозвать незаметно для работы: пока он не заменён, любая копия остаётся действующей неограниченно долго. Ровно этот класс уже дважды снимали федерацией — GitHub → AWS для бэкапа и Vercel → GCP для инсайтов | 🚧 **код готов 2026-09-04, переключение сред не выполнено.** `resolveS3Credentials` выбирает федерацию при заданном `S3_ROLE_ARN` и статические ключи иначе; на Vercel токен OIDC обменивается на временные креды через `AssumeRoleWithWebIdentity`. Приоритет у роли, а не у ключей: во время перехода заданы оба набора, и обратный приоритет означал бы, что испытать федерацию можно только удалив ключи, то есть лишившись отката. Неверный ARN — отказ, а не тихий возврат к ключам: иначе опечатка давала бы работающее приложение, про которое считают, что оно уже ушло с долгоживущих ключей. Имя `S3_ROLE_ARN`, а не документированное Vercel `AWS_ROLE_ARN`, — платформа вправе переопределять `AWS_*` и действительно выставляет `AWS_REGION` по региону исполнения. Ролей две, по одной на среду, с `sub`, прибитым к `environment:production` и `environment:preview`: общая роль с маской пустила бы Preview к боевому бакету и обнулила разделение сред. Границы выбора закреплены `s3-credentials.test.ts`. **Пока не сделано:** OIDC-провайдер и роли в IAM, `S3_ROLE_ARN` в обеих средах, удаление ключей из Vercel и деактивация ключей обоих IAM-пользователей. До этого строка описывает готовность, а не состояние: в проде по-прежнему статический ключ | T* | 2026-09-04 |
