...
production-проверка на serving digest
...
| 3 | Репозиторный VEX для технически доказанных `not_affected` плюс отдельный временный waiver для принятого реального риска | Убирает повторный шум без превращения исключений в способ «просто выкатить» | ✅ Реализовано 08-29 и проверено в production 08-30. CycloneDX 1.6 VEX-профиль и отдельный waiver проходят строгий evaluator по exact CVE/package/version/purl/digest. VEX допускает только `not_affected` с evidence/review; waiver требует owner/approver, approval PR, причины, remediation plan, evidence и истекает максимум через 30 дней. Технические ошибки не подавляются. В run `33285372815` текущих исключений нет: на `sha256:5238cf…2dda1` подавлено 0, осталось 7 Critical + 20 High, поэтому gate ожидаемо красный; отчёты сохранены |
