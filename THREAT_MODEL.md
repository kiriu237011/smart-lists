...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Этап 2a завершён 2026-08-13.** Target guards и no-op миграции прошли для Production и Preview, production promotion дождался migration job того же SHA, Vercel build больше не применяет миграции, а `DIRECT_URL` удалён из обеих сред Vercel. Следующий этап — отдельная runtime-роль без DDL; фактические DB-права и прикладные контроли пока не изменились |
...
| Приложение пока ходит в БД под ролью-владельцем с правами DDL | Риск временно сохраняется: release cutover убрал соседний owner credential, но ещё не сузил сам `DATABASE_URL`. Дампы защищены versioning и проверены на восстановимость, PITR Neon покрывает шесть часов | **Mitigation in progress с 2026-08-12:** release migration и cutover завершены; дальше — runtime least privilege, scoped context и tenant-RLS. Немедленный пересмотр также при втором человеке с доступом к Vercel или снятии whitelist |
