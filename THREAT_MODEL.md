...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Release, runtime, operational roles/backup, scoped ordinary tenant data plane и DB-объекты завершены; RLS/guards для `UserDailyUsage`, `List` и `Item` работают в Preview.** После штатного rollback первый дефект `createList` исправлен additive migration; повторные CI/E2E, exact apply, read-only audit и пользовательский CRUD/sharing smoke зелёные. FORCE отсутствует, rollback готов. Production и оставшиеся пять tenant-таблиц Preview сохраняют role-wide DML. Следующий шаг — подготовить следующую связанную tenant-группу и провести тот же Preview gate |
...
|---|---|---|
| 1 | Поэтапно включить tenant-RLS сначала в Preview | `UserDailyUsage`, `List` и `Item` включены и проверены exact catalog audit и пользовательским CRUD/sharing smoke. Следующая связанная tenant-группа проходит тот же цикл; Production — только после полного Preview go/no-go |
...
| Runtime credential имеет table-wide DML там, где RLS ещё не включён | DDL/ownership уже отозваны в обеих средах, но единая runtime-роль технически может читать и менять чужие строки на оставшихся разрешённых таблицах. Основным live-контролем там остаются `listInSpaceWhere` и ownership-проверки приложения. В Preview `UserDailyUsage`, `List` и `Item` уже ограничены RLS и guard; corrective apply, CRUD/sharing smoke и независимый audit прошли. Production и оставшиеся пять tenant-таблиц Preview всё ещё сохраняют role-wide DML | **Mitigation in progress:** подготовить и поэтапно проверить следующие связанные tenant-группы в Preview с готовым rollback; Production — после полного go/no-go. Немедленный пересмотр при втором человеке с доступом к Vercel, снятии whitelist или появлении прямого SQL-пути из недоверенного ввода |
