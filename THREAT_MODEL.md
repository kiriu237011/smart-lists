...
| **Статические ключи IAM** | env Vercel (prod) и локальный `.env` (dev). Третьим местом был админский ключ на рабочей машине — **отозван 2026-09-03**, CLI переведён на короткоживущие креды `aws login` | У ключей приложения — `PutObject`/`GetObject`/`DeleteObject` на `lists/*`; это и есть весь их blast radius. Отозванный ключ ограничению `lists/*` не подчинялся вовсе: `AdministratorAccess` на аккаунт целиком, включая боевой бакет и дампы `smart-lists-backups`. См. A78 |
...
| A28 | У ключа Vercel нет права `DeleteObjectVersion` | Versioning на вложениях перестаёт быть защитой: тот же ключ уничтожал бы версии | ✅ политика перечисляет ровно `PutObject`/`GetObject`/`DeleteObject`; проверено 08-10. Сломается при любом расширении политики «до кучи» | M | 2026-08-10 |
...
| A80 | Приложение обращается к S3 по короткоживущим credentials, а не по статическому ключу | Ключ IAM в переменных Vercel — единственный оставшийся в проде секрет, который не истекает сам. Его нельзя ни проверить на утечку, ни отозвать незаметно для работы: пока он не заменён, любая копия остаётся действующей неограниченно долго. Ровно этот класс уже дважды снимали федерацией — GitHub → AWS для бэкапа и Vercel → GCP для инсайтов | 🚧 **Preview переведён 2026-09-04, Production — нет.** `resolveS3Credentials` выбирает федерацию при заданном `S3_ROLE_ARN` и статические ключи иначе; на Vercel токен OIDC обменивается на временные креды через `AssumeRoleWithWebIdentity`. Приоритет у роли, а не у ключей: во время перехода заданы оба набора, и обратный приоритет означал бы, что испытать федерацию можно только удалив ключи, то есть лишившись отката. Неверный ARN — отказ, а не тихий возврат к ключам: иначе опечатка давала бы работающее приложение, про которое считают, что оно уже ушло с долгоживущих ключей. Имя `S3_ROLE_ARN`, а не документированное Vercel `AWS_ROLE_ARN`, — платформа вправе переопределять `AWS_*` и действительно выставляет `AWS_REGION` по региону исполнения. Ролей две, по одной на среду, с `sub`, прибитым к `environment:production` и `environment:preview`: общая роль с маской пустила бы Preview к боевому бакету и обнулила разделение сред. Границы выбора закреплены `s3-credentials.test.ts`. **Preview:** провайдер и обе роли заведены, `S3_ROLE_ARN` задан, статические ключи из среды удалены — и вложения продолжают работать, то есть использоваться может только роль. Подтверждено со стороны AWS: `RoleLastUsed` роли `smart-lists-vercel-preview` — 2026-09-04, регион `ap-northeast-1`. Отдельного доказательства среды не требуется: trust policy сравнивает `sub` через `StringEquals`, поэтому сам факт успешного assume означает, что токен нёс ровно `environment:preview`. CloudTrail в этот момент ещё молчал — Event history отстаёт, и полагаться на неё как на единственный признак нельзя. **Не сделано:** Production по-прежнему на статическом ключе; там же остаётся деактивация ключа боевого IAM-пользователя. Ключ dev-пользователя останется живым и после перехода — его использует локальная разработка, у которой OIDC нет | T* | 2026-09-04 |
