...
| Runtime credential до RLS имеет table-wide DML в разрешённых таблицах | DDL/ownership уже отозваны в обеих средах, но единая runtime-роль технически может читать и менять чужие строки там, где DML разрешён. Основным live-контролем остаются `listInSpaceWhere` и ownership-проверки приложения. Scoped data plane, общая access-функция, policies и column guards применены в обеих БД; Alice/Bob-тесты под restricted runtime прошли. Preview live audit `32446720820` подтвердил точный catalog, но пока RLS выключен, эти объекты не ограничивают direct table DML | **Mitigation in progress:** отдельными группами включить и проверить enforcement в Preview; Production только после полного go/no-go. Немедленный пересмотр при втором человеке с доступом к Vercel, снятии whitelist или появлении прямого SQL-пути из недоверенного ввода |
