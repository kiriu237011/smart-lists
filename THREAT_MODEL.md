...
2. **Postgres least privilege и RLS:** архитектурный проект согласован
   ...
   обязательный Deployment Check настроен в Vercel, но ещё не проверен
   контрольным release; runtime без DDL, scoped-контекст и RLS ещё не внедрены.
...
| A32 | Vercel удерживает production alias до успешной DB migration job того же SHA | Без gate новый код может стать доступен со старой схемой, даже если миграции вынесены в GitHub | 🟡 **настроено, но ещё не доказано**: в Vercel вручную добавлен обязательный GitHub check `Production database migration`; нужен контрольный release, подтверждающий, что production alias действительно ждёт job того же SHA. До этого `build:deploy` сохраняется |
...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Этап 2a доказан 2026-08-13 для Production и Preview.** Target guards прошли, обе миграции были no-op, Preview push и Vercel deployment успешны. `build:deploy` сохраняется до ручного go/no-go по A32; текущие DB-права и прикладные контроли пока не изменились |
