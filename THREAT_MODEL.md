...
production-проверка на serving digest и reviewed VEX по 18 неглибсишным CVE
...
| 3 | Репозиторный VEX для технически доказанных `not_affected` плюс отдельный временный waiver для принятого реального риска | Убирает повторный шум без превращения исключений в способ «просто выкатить» | ✅ Реализовано 08-29 и проверено на первом реальном VEX 08-30. CycloneDX 1.6 VEX-профиль и отдельный waiver проходят строгий evaluator по exact CVE/package/version/purl/digest. VEX допускает только `not_affected` с evidence/review; waiver требует owner/approver, approval PR, причины, remediation plan, evidence и истекает максимум через 30 дней. Технические ошибки не подавляются. PR FastAPI №38 записал 18 доказанных CVE / 21 exact package match для `sha256:082760…52fe3`; post-merge run `33298309218` подтвердил VEX=21, waiver=0 и сохранил красный gate на оставшихся 2 Critical + 4 High glibc-находках |
