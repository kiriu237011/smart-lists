...
|---|---|---|---|
| 1 | Еженедельный fail-closed `grype` по digest всех traffic/tagged Cloud Run revisions | Невидимость CVE между выкладками; SBOM для этого не требуется | ✅ Код, тесты и отдельная read-only IAM identity готовы 08-29. После попадания workflow в `main`: первый ручной запуск обязан корректно показать текущие находки или технический отказ; operational-критерий — четыре последовательных scheduled run без пропуска |
| 2 | Syft генерирует CycloneDX JSON 1.6 и прикрепляет его к digest в Artifact Registry | Даёт машиночитаемую опись именно собранного образа. История удалённых образов намеренно не хранится: attachment удаляется вместе с target image | Не начат; только после полного закрытия этапа 1 |
| 3 | Репозиторный VEX для технически доказанных `not_affected` плюс отдельный временный waiver для принятого реального риска | Убирает повторный шум без превращения исключений в способ «просто выкатить» | Не начат; правило обязано быть привязано к CVE, package/version и image digest, иметь evidence/review; `under_investigation`, blanket ignore и ложный `not_affected` не подавляются |
| 4 | Зафиксировать эксплуатационную границу | Не даёт спутать опись, сканер и систему управления портфелем | Dependency-Track не внедряется: для одного контейнерного сервиса это overkill. Grype работает напрямую с image/CycloneDX. Provenance остаётся отдельной задачей |
