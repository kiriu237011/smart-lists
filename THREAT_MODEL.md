...
Распределение на 2026-09-01: **T — 32, M — 23, — — 9, R — 7, T\* — 2**. Практический вывод в том, что почти половина реестра (30 строк: M и R) держится на доказательстве, которое устаревает без единого сигнала.

**Чем эта разметка не является.** Метка `T` при первичной разметке проставлялась по факту существования названного теста; содержимым проверены выборочно те строки, что попадали под два признака риска — утверждение о полноте и тест, не названный поимённо. Так была найдена A29, оказавшаяся ложной. Из этого следует, что среди строк `T` может быть ещё одна такая же: тест существует и проходит, но закрепляет не то, что утверждает строка. Колонка отвечает на вопрос «что заметит поломку», и сама по себе не является доказательством, что допущение истинно сегодня. Доверификация `T` содержимым — отдельная незакрытая работа, и до неё `T` следует читать как «заявлен автотест», а не как «проверено».
