...
| ~~`INSIGHTS_SERVICE_SECRET`~~ | **удалён 2026-08-09**, последние копии убраны 2026-09-02 | Актива больше нет: аутентификация перешла на короткоживущий ID-токен, ротировать нечего. Переменная пережила протокол на три недели в локальном `.env` и в двух тестовых окружениях — мёртвое значение выглядит как действующий секрет, поэтому удалено и оно; неиспользуемость закреплена тестом, который подставляет переменную обратно |
| **Статические ключи IAM** | env Vercel (prod), локальный `.env` (dev) и — **найдено 2026-09-02** — отдельный ключ в открытом CSV на рабочей машине | У ключей приложения — `PutObject`/`GetObject`/`DeleteObject` на `lists/*`. Ключ из CSV шире: он перечисляет и боевой бакет вложений, и бакет дампов `smart-lists-backups`, то есть не подчиняется ограничению `lists/*`, на котором построено утверждение о равенстве прав сред. См. A77 |
...
| A77 | На рабочей машине нет действующих секретов вне проектного `.env` | A23 объявляет машину доверенной, но отвечает за то, что на ней **исполняется**; что на ней **лежит файлами** — не ограничивало ничто. Копия секрета не участвует в ротации: она переживает смену того значения, которое дублирует, и не попадает ни в один инвентарь. Отличить живую копию от мёртвой чтением нельзя — только пробой | ❌ **найдено 2026-09-02**: рядом с репозиторием лежали четыре файла. Два проверены и оказались мертвы — production-строка Neon под `neondb_owner` (`28P01`), секрет Google OAuth (`invalid_client`, при контрольной пробе действующего клиента — `invalid_grant`) и приложение Pusher, которого нет ни в одном кластере; удалены. Два остаются действующими: ключ IAM в CSV, видящий боевой бакет и бакет дампов, и коды восстановления MFA Vercel. Оба закрываются только в консолях — отзыв ключа и перевыпуск кодов. Отдельный остаток: доказать, что production `AUTH_SECRET` не равен лежавшему в удалённой копии, нечем — значение в Vercel не читается, поэтому дешевле перезаписать, чем выяснять | — | 2026-09-02 |
