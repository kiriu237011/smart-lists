...
7. **Сетевой контейнмент исходящего:** ни Vercel, ни Cloud Run не ограничивают
   ...
   держится кодом: адреса всех исходящих запросов константны — см. A68 и A69.
...
| A68 | Адрес каждого серверного исходящего запроса Next.js задан кодом или переменной окружения, но не данными пользователя | Egress рантайма Vercel не ограничен ничем, и ограничить его нечем: фильтрации исходящего по адресатам у Vercel нет ни на одном тарифе, а Secure Compute из Enterprise выдаёт статические исходящие адреса, то есть меняет источник, а не разрешённые направления. Поэтому SSRF в Server Action уходил бы на произвольный хост, и остановить его было бы нечем — сеть в этой цепочке не участвует вовсе. Это зеркало A56 для второго рантайма: там же, где у сервиса есть закреплённый тестом контракт, у Next.js его нет | ⚠️ **проверено 2026-08-31.** Серверный исходящий запрос в приложении ровно один — `fetch` по `${serviceUrl}/insights` в `insights.ts`, адрес приходит из `INSIGHTS_SERVICE_URL`. Второй `fetch` в `Attachments.tsx` исполняется в браузере и к egress рантайма отношения не имеет. Пользовательские данные в адрес не попадают нигде. **Остаток осознан:** `INSIGHTS_SERVICE_URL` — щель той же формы, что закрытая в A56 `ANTHROPIC_BASE_URL`, но заметно уже. Значение задаёт и адрес запроса, и `audience` ID-токена (`getCloudRunIdToken(serviceUrl)`), поэтому подменённый адрес получил бы токен, выписанный на него самого: предъявить его настоящему сервису нельзя — тот сверяет `aud` со своими URL из `SERVICE_AUDIENCE`. Ущерб ограничен телом запроса, то есть содержимым списка, и раскрытием identity вызывающего в claims. Вдобавок изменение переменной на Vercel вступает в силу только с новым деплоем, тогда как правка env ревизии Cloud Run деплоем не выглядит. Тестом строка не закреплена — в отличие от A56 |
...
| Исходящий трафик Vercel не ограничен, и ограничить его нечем | Egress-фильтрации по адресатам у Vercel нет ни на одном тарифе — это не ограничение `hobby`, а отсутствующий продукт. Значит выбор стоит не между «ограничить» и «не ограничить», а между текущим хостингом и переездом рантайма, и цена такого переезда несопоставима с угрозой при одном владельце и whitelist из знакомых. Компенсация: адрес единственного серверного запроса не выводится из пользовательских данных (A68), установка не исполняет чужой код (A51), версии выдерживаются неделю (A52) | Появление серверного запроса по адресу из пользовательского ввода; переезд рантайма; появление у Vercel средств egress-контроля |
