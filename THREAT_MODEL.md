...
target guards и успешные migration runs Preview/Production
...
2. **Postgres least privilege и RLS:** архитектурный проект согласован
  2026-08-12. Release-контур этапа 2a проверен no-op миграциями Production и
   ...
   Preview/Production 2026-08-21. RLS и guards в live-средах не включены.
   Матрица и gate:
...
|---|---|---|
| 1 | Поэтапно включить tenant-RLS сначала в Preview | Policies и disabled guards применены в обеих БД; реальный DB-контроль появится только после catalog audit, включения и отрицательной live-проверки |
...
| Runtime credential до RLS имеет table-wide DML в разрешённых таблицах | DDL/ownership уже отозваны в обеих средах, но единая runtime-роль технически может читать и менять чужие строки там, где DML разрешён. Основным live-контролем остаются `listInSpaceWhere` и ownership-проверки приложения. Scoped data plane, общая access-функция, policies и column guards применены в обеих БД; Alice/Bob-тесты под restricted runtime прошли. Но пока live RLS выключен, эти объекты не ограничивают direct table DML | **Mitigation in progress:** выполнить live catalog audit, затем отдельными группами включить и проверить enforcement в Preview; Production только после полного go/no-go. Немедленный пересмотр при втором человеке с доступом к Vercel, снятии whitelist или появлении прямого SQL-пути из недоверенного ввода |
