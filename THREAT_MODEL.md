...
**Последняя сверка с кодом:** 2026-08-11
...
- приоритет 2 закрыт, кроме осознанно принятого риска роли Postgres с DDL;
...
2. **Postgres least privilege:** отдельная runtime-роль без DDL имеет смысл
   только после переноса миграций из сборки Vercel в отдельный CI-шаг.
...
Это актуальный backlog, а не список уже закрытых находок:

| Порядок | Действие | Что меняет |
|---|---|---|
| 1 | Перенести `prisma migrate deploy` из сборки Vercel в отдельный CI-шаг, затем выдать runtime отдельную роль Postgres без DDL | Делает least privilege настоящим: владельческая строка перестаёт лежать рядом с runtime |
