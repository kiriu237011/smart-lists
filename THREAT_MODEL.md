...
Распределение на 2026-09-01 после первого прохода доверификации: **T — 29, M — 23, — — 9, R — 7, T\* — 5**. Практический вывод в том, что почти половина реестра (30 строк: M и R) держится на доказательстве, которое устаревает без единого сигнала.
...
Три строки проход не прошли и переведены в `T*`: **A13** (проверяется install-шаг migration job, а не отсутствие `DIRECT_URL` у обычных job), **A56** и **A68** (закреплены адреса существующих вызовов, но не утверждение «ровно три» и «ровно один» — новый исходящий запрос прогон не покрасит). Все три — та же форма, что A29: строка утверждает полноту, которую ничто не обеспечивает.

Не верифицированы содержимым: A30, A35, A36, A38, A40, A53, A62, A64, A65, A66, A67. Это строки о конкретных сценариях, а не о полноте, поэтому в первом проходе они пропущены осознанно; до второго прохода их `T` следует читать как «заявлен автотест», а не как «проверено».
