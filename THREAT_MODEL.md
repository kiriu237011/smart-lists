...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Release, runtime и operational owner/migrator/backup этапы завершены 2026-08-13; foundation, `spaces`, server read-path, user quota, space mutations, DB-фазы AI insights/attachments, ListGroup lifecycle/membership, List lifecycle и sharing lifecycle реализованы 2026-08-14.** Прямой allowlist сократился с 13 до 6; частично перенесённый `index.ts` дополнительно защищён per-action guard для четырнадцати group/list/sharing actions. Sharing больше не создаёт default-space от имени получателя: migration/Auth.js гарантируют строку, а составной FK даёт fail-closed rollback без privileged helper; чужой listId также не раскрывает существование email. DB-тесты доказывают ownership/share-space semantics, конкурентную атомарность, atomic rebalance, cross-space отказ и commit до внешних эффектов. S3/AI/Pusher не удерживают DB-транзакции; attachment/list/sharing передают заранее собранных получателей в `after()` без tenant-чтения. Для глобальной attachment-квоты и cross-space stale-cleanup до enforcement нужен узкий DB-helper — обычная space-policy изменила бы семантику. Остальные tenant-мутации и специальные потоки ещё не перенесены, RLS выключен, поэтому фактическая DB-изоляция строк пока не завершена. Следующие шаги — перенос оставшихся потоков, helper/policies и поэтапное включение tenant-RLS |
