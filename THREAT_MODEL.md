...
репозиторная политика VEX/waiver с exact-match gate
...
| 3 | Репозиторный VEX для технически доказанных `not_affected` плюс отдельный временный waiver для принятого реального риска | Убирает повторный шум без превращения исключений в способ «просто выкатить» | ✅ Реализовано 08-29. CycloneDX 1.6 VEX-профиль и отдельный waiver проходят строгий evaluator по exact CVE/package/version/purl/digest. VEX допускает только `not_affected` с evidence/review; waiver требует owner/approver, approval PR, причины, remediation plan, evidence и истекает максимум через 30 дней. Технические ошибки не подавляются. Текущих исключений нет; реальный scan остался красным |
