...
Масштаб доверия измерим: в `package-lock.json` 906 пакетов при 38 прямых, в
...
| Контроль | Где | Строка |
|---|---|---|
| Установка не исполняет чужой код | `npm ci --ignore-scripts`; `pip --only-binary=:all:` | A51 |
| Новая версия не принимается в день публикации | `cooldown.default-days: 7`, автомержа нет, мажоры вручную | A52 |
| Целостность артефакта | `integrity` sha512 у 900 записей lock из 906; `pip --require-hashes` | A50 |
