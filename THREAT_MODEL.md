...
Снимок живого GCP IAM от 2026-08-29, переподтверждён по консоли 2026-09-03.
У всех перечисленных identities отсутствуют user-managed JSON-ключи.
...
|---|---|---|
| **OAuth-токены Google** (`Account.access_token`, `refresh_token`, `id_token`) | Neon, а также в каждом дампе `backups/*` | Единственный актив, чей ущерб **выходит за пределы Smart Lists** — даёт доступ к Google-аккаунту пользователя. С 2026-09-05 копия в дампах выведена из досягаемости административной сессии аккаунта AWS политикой бакета (A82); копия в Neon по-прежнему достижима владельцу доступа к консоли (A43, A44) |
...
| A7 | Сервис не вызывается анонимно | Rate limit FastAPI перестаёт существовать, флуд оплачивается из своего кармана | ✅ **закрыто 08-09**: `run.invoker` только у `vercel-insights-invoker`, анонимно 403. Ingress остаётся `all` — это осознанно, Vercel живёт вне GCP | M | 2026-08-09 |
...
| A24 | В спецификациях ревизий Cloud Run нет значений секретов | Секрет читается из истории и после «удаления» из текущей ревизии | ✅ **закрыто 08-10**: 24 старые ревизии удалены после отзыва ключа, живая одна. Секретов в её спецификации нет вовсе, поэтому строка больше не может сломаться накоплением истории | M | 2026-08-10 |
...
| A26 | Личность сервиса в Cloud Run не меняется молча | Правило федерации привязано к `sub` и `email` `insights-api-runtime`; смена SA = отказ Anthropic без внятной причины | ✅ `deploy.yml` передаёт `--service-account` явно, правило зафиксировано в `AGENTS.md` сервиса | R | 2026-08-10 |
| A27 | Токен федерации ограничен одним workspace | Компрометация даёт доступ ко всей организации Anthropic | ⚠️ правило включено ровно в один workspace и `workspace_id` передаётся явно. Scope при этом `workspace:developer` — столько же, сколько давал прежний ключ; сузить нельзя, `workspace:inference` правилу недоступен, а запрос такого scope при обмене сервер игнорирует (проверено 08-10) | — | 2026-08-10 |
...
| A19 | Никто, кроме owner, не имеет доступа к проекту GCP и аккаунту AWS | Plain-env секреты и админские права становятся доступны второму лицу | ✅ **проверено 08-09**: в IAM проекта один человек; ср. триггеры пересмотра | M | 2026-08-09 |
...
| A82 | Дампы БД недостижимы из повседневной сессии, даже административной | В каждом дампе лежат `Account.access_token` и `refresh_token` пользователей Google — единственный актив, чей ущерб выходит за пределы приложения. Пока доступ к дампам следовал из `AdministratorAccess`, кража 12-часовой сессии `aws login` означала доступ к Google-аккаунтам пользователей, а не потерю хобби-проекта | ✅ **введено 2026-09-05.** Политика бакета `smart-lists-backups` запрещает чтение и удаление объектов всем, кроме root; запись — всем, кроме root и роли бэкапа; и отдельно `PutBucketPolicy`, `DeleteBucketPolicy`, `PutReplicationConfiguration`, `PutLifecycleConfiguration`, `PutBucketVersioning` — всем, кроме root. Последний statement закрывает обходы: снять запрет, вывезти репликацией, уничтожить ускоренным истечением, ослабить версионирование. Запрет resource-based и потому не зависит от прав в IAM: сессия с `AdministratorAccess` может завести себе роль с любыми правами и всё равно получит отказ. Проверено на боевом бакете — чтение, удаление и снятие политики отвергнуты с «explicit deny in a resource-based policy». `ListBucket` намеренно оставлен: независимая проверка того, что бэкапы идут, ценнее сокрытия имён файлов, а прецедент скрытно пропускавшейся release-job (пункт плана 1.5) показывает, чего стоит доверять одному лишь зелёному CI. Lifecycle не затронут — истечение выполняется внутренними эндпоинтами S3, а не вызовом `DeleteObject`. Право записи роли бэкапа подтверждено ручным прогоном workflow: новый дамп лёг в бакет поверх занятого ключа. **Восстановление доступа — только через root**, и путь отрепетирован на одноразовом бакете до применения к боевому. Права `K` при этом не сужались: дешевле вынести ценное из досягаемости, чем строить разделение ролей, трение которого обходят. Боевой бакет вложений так не закрывался — приложение читает его штатно, конструкция там сложнее, а утечка вложений за пределы приложения не выходит | M | 2026-09-05 |
| A83 | Каждый credential БД ограничен местом, где его разрешено использовать | `DIRECT_URL` ролей migrator лежит в GitHub Environments с политикой ветки `main` — его получает только job, объявившая соответствующий Environment. Credential роли `smartlists_backup` до сих пор repository-level, то есть доступен любому workflow на любой ветке: достаточно добавить в ветку файл, который его прочитает. Право на это сегодня есть только у владельца, но ограничение по месту использования — единственное, чего у этого секрета нет, а у двух других есть | 🚧 **подготовлено 2026-09-05, перенос не выполнен.** Выяснилось, что это не правка одной строки: добавление `environment:` к job меняет OIDC-claim `sub` с `...:ref:refs/heads/main` на `...:environment:Backup`, и трастовая политика роли AWS, прибитая к первому значению через `StringEquals`, отвергла бы второе — бэкапы сломались бы в тот же день. Формат подтверждён по API (`use_default: true`, префикс в ID-формате), а не по памяти. Сделано: политика роли `smart-lists-backup-ci` принимает **оба** `sub`, как при переходе S3 на федерацию, — старый путь работает, новый заранее разрешён, откат есть на каждом шаге. Действующий путь проверен ручным прогоном после правки. **Не сделано:** Environment `Backup` с политикой ветки `main`, перенос значения, удаление repo-secret, сужение трастовой политики обратно до одного `sub`. Перенос отложен намеренно: GitHub не отдаёт значение секрета обратно, поэтому «перенести» означает «ввести заново», а это та же операция, что и ротация. Разделив их, мы дважды достали бы пароль роли из консоли и дважды провели через буфер обмена; объединение выполняется в подшаге ротации | M | 2026-09-05 |
...
Распределение на 2026-09-01 после обоих проходов доверификации, закрытия A68 и A56 и появления A74 и A75: **T — 32, M — 23, — — 9, R — 7, T\* — 4** (75 строк). Практический вывод в том, что почти половина реестра (30 строк: M и R) держится на доказательстве, которое устаревает без единого сигнала.
...
| 1.5 | Починить release-job production-миграций: она перестала запускаться | Mitigate | 🔴 **найдено 2026-09-04, не исправлено. Требует перепроверки до правки.** `Production database migration` уходит в `skipped` во всех 14 последних прогонах на `main` (2026-09-02 … 09-04, включая merge #172). Это регрессия, а не изначальная поломка: A32 доказана прогоном 2026-08-13, где job стартовала и успешно завершилась. Промежуток между 08-13 и 09-02 не выбирался, поэтому граница регрессии не установлена. Условия на момент проверки истинны все: `event=push`, `ref=refs/heads/main`, repo-переменная `ENABLE_PRODUCTION_MIGRATION=true`, шесть job из `needs` зелёные, сама job стартует после `gate` и тут же пропускается. Рабочая гипотеза: флаг проверяется на уровне **job**, а job объявляет `environment: Production`, и `vars` там резолвится в области окружения, где переменных нет вовсе. Косвенное подтверждение — `sync-preview.yml`: тот же флаг, но проверка на уровне **шага**, и `Apply Preview migrations` 2026-09-04 отработал успешно. Подозреваемое изменение — `f2bac03` (2026-08-28, docs-only fast path), перестроивший условия job. **Гипотеза объясняет наблюдения, но не доказана; чинить release-механику по догадке нельзя** — сперва воспроизвести причину, затем править, затем подтвердить реальным прогоном на `main`. Ущерба пока нет: новых миграций не было. Первая же уехала бы в прод неприменённой, а деплой прошёл бы — для required check «пропущено» и «успешно» неразличимы. Затрагивает A32 и утверждение `DATABASE_SECURITY.md` о том, что production-миграции применяет эта job |
