...
| A39 | У модели нет никаких возможностей помимо генерации текста | Инструменты, MCP-серверы, `container` и beta-заголовки — единственный способ дать модели сеть, выполнение кода или доступ к metadata-серверу; с ними prompt injection перестала бы быть ограничена текстом ответа | ✅ **закреплено 08-14**: тест сервиса сверяет набор аргументов `messages.create` целиком (`model`, `max_tokens`, `system`, `messages`) и требует отсутствия позиционных. **Форма проверки изменена 2026-09-03 вместе с появлением `INSIGHTS_MODEL`.** Набор аргументов перестал быть одним и тем же для всех конфигураций, поэтому «целиком» проверяется теперь в каждой отдельно: с `haiku` набор прежний, с `sonnet` к нему добавляется ровно один ключ `thinking={"type": "disabled"}`. Этот ключ управляет генерацией и способностей не даёт — ни инструментов, ни сети, ни выполнения кода, — а передан он явно по обратной причине: Sonnet 5 при пропуске аргумента включает adaptive thinking сам, и черновик начинает делить `max_tokens` с ответом. Само допущение при этом усилено, а не ослаблено: раньше полнота держалась на перечислении известного вызова, теперь набор допустимых ключей (`model`, `thinking`) сверяется обходом самого реестра `MODEL_CHOICES`, поэтому новая запись с `tools`, `betas`, `mcp_servers` или `container` красит прогон и без отдельного теста под неё. Проверено негативно: `tools` с web search, временно добавленный в запись `sonnet`, роняет и сторож реестра, и проверку конфигурации. Переменная заведена ради сравнения моделей на настоящих списках и уедет вместе со сравнением; неизвестное значение не выбирает модель, а откатывается к default; на уровне организации web search и Managed Agents при этом включены — на поток инсайтов не влияет, инструменты объявляются в запросе, но blast radius украденной identity ими не сужен | T | каждый CI |
