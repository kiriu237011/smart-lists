...
Приоритет 1.3a закрыт: 2026-08-09 — shared secret упразднён, а не перенесён
...
| A24 | В спецификациях ревизий Cloud Run нет значений секретов | Секрет читается из истории и после «удаления» из текущей ревизии | ❌ **ложно**: `ANTHROPIC_API_KEY` лежит открытым в 19 старых ревизиях. Закрывается связкой 1.3b + 1.3d |
...
| 1.3b | `ANTHROPIC_API_KEY` в Secret Manager, с ротацией и отзывом старого ключа | Mitigate | Единственный оставшийся настоящий секрет. Отзыв обязателен: значение остаётся в спецификациях старых ревизий, пока они не удалены |
| 1.3c | Сузить `github-deployer` с `run.admin` до `run.developer`, отделить runtime-идентичность сервиса от деплойной | Mitigate | Разрывает «право деплоить ⇒ право прочитать секрет». Сам по себе Secret Manager этого не даёт: кто может выкатить ревизию, может выкатить и код, читающий секрет |
| 1.3d | Удалить старые ревизии Cloud Run | Mitigate | Ревизия неизменяема: удаление переменной создаёт новую, а не правит старые. Делать после 1.3b, иначе история вычищается и тут же заводится заново |
...
**Самое дешёвое решение оказалось удалением, а не переносом.** Пункт плана звучал как «перенести оба секрета в Secret Manager». Один из них после разбора выяснилось, что можно не переносить, а упразднить: он отвечал на тот же вопрос, что и IAM, только хуже. Хранилище секретов — правильный ответ на вопрос «где держать секрет»; вопросу «нужен ли он вообще» оно предшествовать не должно.
