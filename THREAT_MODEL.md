...
репозитория Artifact Registry, состав env живой ревизии
...
- цепочка поставок: установка не исполняет чужой код на всех шести поверхностях,
  ...
  обоих репозиториях. Открыто — provenance не проверяется, а уязвимость,
  опубликованная между выкладками образа сервиса, не видна никому; последнее
  ведётся отдельной задачей из четырёх этапов;
...
| Внешняя инфраструктура | repository contracts, target guards и периодические live audits | IAM, CORS, versioning, Force TLS и часть Neon/GitHub settings могут дрейфовать между ручными сверками |
...
6. **Между выкладками об образе не знает никто.** grype в `deploy.yml` видит
   образ в момент сборки; уязвимость, опубликованная после, не видна до
   следующей выкладки. Отдельная задача ниже.

### SBOM и непрерывное сканирование образа (задача, 4 этапа)
...
уязвимости образа: на 08-20 grype показывал 2 critical и 2 high, все четыре в
`perl-base`. Ни `npm audit`, ни Dependency Review этот слой не видят в
принципе.
...
| Этап | Действие | Что закрывает | Условие перехода к следующему |
|---|---|---|---|
| 1 | Еженедельный `grype IMAGE@DIGEST` по расписанию | Невидимость CVE между выкладками — то есть пункт 6 выше целиком. Ни SBOM, ни нового формата не требует | Проработал месяц и выдал хотя бы одну находку, на которую было принято решение |
| 2 | `sbom: true` в `docker/build-push-action` | Опись переживает cleanup policy Artifact Registry, которая удаляет образы старше 30 дней. Через полгода на вопрос «что было в образе от такого-то числа» появляется ответ | Этап 1 работает, и понадобилось ответить на вопрос об уже удалённом образе |
| 3 | VEX для разобранных находок | Разбор вида «CVE есть, но путь недостижим» перестаёт быть прозой, которую надо перечитывать при каждой сверке, и становится машиночитаемым подавлением. Пример такой прозы уже есть — строка про `deepmerge-ts` в принятых рисках | Этап 1 начал давать повторяющийся уже разобранный шум |
| 4 | CycloneDX / Dependency-Track | Централизованный учёт, «в каких развёрнутых артефактах есть этот пакет» | Вероятно, никогда: инструмент для десятков сервисов, а у нас два |

Отдельно: **provenance и attestation так и не проверяются** (пункт 2 выше). У
