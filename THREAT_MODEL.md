...
Распределение на 2026-09-01 после первого прохода доверификации и закрытия A68: **T — 30, M — 23, — — 9, R — 7, T\* — 4**. Практический вывод в том, что почти половина реестра (30 строк: M и R) держится на доказательстве, которое устаревает без единого сигнала.
...
Три строки проход не прошли — все той же формы, что A29: строка утверждает полноту, которую ничто не обеспечивает. **A68** закрыта в тот же день обходом `src` (`outbound-requests.test.ts`) и вернулась в `T`. Остаются `T*`: **A13** (проверяется install-шаг migration job, а не отсутствие `DIRECT_URL` у обычных job) и **A56** — зеркало A68 на стороне сервиса, где «ровно три» по-прежнему держится чтением кода, а не обходом исходников.
