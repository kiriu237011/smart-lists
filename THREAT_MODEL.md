...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Release, runtime, operational roles/backup, scoped ordinary tenant data plane и DB-объекты завершены; первый enforcement-canary работает в Preview.** `UserDailyUsage` прошла локальные enable/rollback и fail-closed тесты, PR №107/main CI, live переход `disabled → usage-canary`, пользовательский CRUD smoke и независимый read-only audit. Следующий профиль `List + Item` подготовлен локально: configurator разрешает только линейный переход и откат, проверяет точные policy predicates и тела/ACL helper/guard, а restricted-role suite и отдельный partial-profile Server Action test дали 289 зелёных DB-тестов. Live RLS/guard пока включены только на `UserDailyUsage` в Preview, FORCE отсутствует; Production и остальные семь tenant-таблиц сохраняют role-wide DML. Следующий шаг — implementation PR/CI, затем отдельный Preview live gate |
...
|---|---|---|
| 1 | Поэтапно включить tenant-RLS сначала в Preview | Первый `UserDailyUsage` canary включён и проверен; `List + Item` прошёл локальный design/rollback gate и ждёт PR/CI, затем отдельного live apply; Production — только после полного Preview go/no-go |
...
| Runtime credential имеет table-wide DML там, где RLS ещё не включён | DDL/ownership уже отозваны в обеих средах, но единая runtime-роль технически может читать и менять чужие строки на оставшихся разрешённых таблицах. Основным live-контролем там остаются `listInSpaceWhere` и ownership-проверки приложения. В Preview `UserDailyUsage` уже ограничена RLS и guard; live apply, CRUD smoke и независимый audit прошли. `List + Item` пока лишь локально подготовлены, поэтому Production и остальные семь tenant-таблиц всё ещё сохраняют role-wide DML | **Mitigation in progress:** опубликовать и проверить `List + Item`, затем включить его отдельным Preview gate с готовым `list-item → usage-canary` rollback; Production — после полного go/no-go. Немедленный пересмотр при втором человеке с доступом к Vercel, снятии whitelist или появлении прямого SQL-пути из недоверенного ввода |
