...
| ~~`INSIGHTS_SERVICE_SECRET`~~ | **удалён 2026-08-09**; копии в репозитории и локальном `.env` убраны 2026-09-02, состав переменных Vercel не сверялся | Актива больше нет: аутентификация перешла на короткоживущий ID-токен, ротировать нечего. Переменная пережила протокол на три недели в локальном `.env` и в двух тестовых окружениях — мёртвое значение неотличимо от действующего, поэтому удалено и оно; неиспользуемость закреплена тестом, который подставляет переменную обратно. Утверждать, что копий не осталось нигде, нельзя: переменные Vercel перечисляются только вручную через `vercel env ls`, и эта сверка не выполнялась |
| **Статические ключи IAM** | env Vercel (prod) и локальный `.env` (dev). Третьим местом был админский ключ на рабочей машине — **отозван 2026-09-03**, CLI переведён на короткоживущие креды `aws login` | У ключей приложения — `PutObject`/`GetObject`/`DeleteObject` на `lists/*`; это и есть весь их blast radius. Отозванный ключ ограничению `lists/*` не подчинялся вовсе: `AdministratorAccess` на аккаунт целиком, включая боевой бакет и дампы `smart-lists-backups`. См. A77 |
...
| A20 | Только Production обращается к AI-сервису | Доступ к общему сервису расходится по средам | ✅ провайдер OIDC пускает единственную среду `production`; в Preview не задан `INSIGHTS_SERVICE_URL` | M | 2026-08-09 |
...
| A77 | На рабочей машине нет действующих секретов вне проектного `.env` | A23 объявляет машину доверенной, но отвечает за то, что на ней **исполняется**; что на ней **лежит файлами** — не ограничивало ничто. Копия секрета не участвует в ротации: она переживает смену того значения, которое дублирует, и не попадает ни в один инвентарь. Отличить живую копию от мёртвой чтением нельзя — только пробой | ⚠️ **найдено 2026-09-02, закрыто наполовину 2026-09-03**. Рядом с репозиторием лежали четыре файла; два проверены пробой и оказались мертвы — production-строка Neon под `neondb_owner` (`28P01`), секрет Google OAuth (`invalid_client`, при контрольной пробе действующего клиента — `invalid_grant`) и приложение Pusher, которого нет ни в одном кластере; удалены. **Сам обход был неполон:** пятым местом тот же ключ лежал в `~/.aws/credentials` и нашёлся только 09-03, когда его отзыв сломал CLI. Место хранения секрета находится не осмотром каталогов, а вопросом «какой инструмент его читает». Ключ оказался тяжелее, чем предполагала строка активов: не доступ к бакетам, а credential IAM-пользователя `K` из группы `Admins` с `AdministratorAccess`, то есть полные права на аккаунт `777078924813` — с боевым бакетом вложений и бакетом дампов; открытым текстом он лежал с 2026-07-28 по 2026-09-03. Отозван, обе копии удалены, CLI переведён на `aws login`: OAuth 2.0 с PKCE, креды на 15 минут с авторефрешем до 12 часов, отзыв через `aws logout`, входы видны в CloudTrail. Коды восстановления MFA Vercel перевыпущены, старый файл удалён. Файловая часть на этом закрыта: ни одного из пяти файлов больше нет. **Допущение при этом остаётся ложным — теперь по другой причине.** Полная сверка известных хранилищ 09-03 показала, что предъявители на машине никуда не делись, просто принадлежат сторонним CLI: `~/.config/neon/credentials.json` хранит `refresh_token` к Neon (бессрочный до отзыва — это и есть файл, который A23 называла, но не локализовала; по A44 доступ к консоли Neon равен компрометации БД), а `%APPDATA%/gcloud/credentials.db` — учётные данные к проекту GCP с AI-сервисом и OAuth-клиентами, и эту копию не называла ни одна строка реестра. Оба долгоживущие. Единственный ограниченный по времени — AWS: `~/.aws/login/cache`, максимум 12 часов. Чисто у `gh` (keyring) и Docker (`credsStore: desktop`, инлайн-`auths` пуст); `.npmrc`, `.git-credentials` и токена Vercel CLI нет вовсе. **Остаток по значениям:** доказать, что production `AUTH_SECRET` не равен лежавшему в удалённой копии, нечем — значение в Vercel не читается, поэтому дешевле перезаписать, чем выяснять | — | 2026-09-03 |
