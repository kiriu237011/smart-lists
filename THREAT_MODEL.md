...
**Путь образа сервиса** — отдельный участок той же границы: `Dockerfile` → GitHub Actions → Artifact Registry → Cloud Run. Разобран 2026-08-20 сканированием фактически собранного и фактически задеплоенного образа. Строки реестра A46–A48 фиксируют результат: происхождение образов в compose, актуальность пакетов базового Debian и адресацию выкладки по digest. Что осталось незакрытым — непрерывное наблюдение за уже опубликованным образом; см. принятые риски.
