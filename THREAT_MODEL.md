...
| A67 | Cloud Run получает только image digest с проверяемым происхождением из доверенного production workflow | SHA-тег и доступ к Artifact Registry не доказывают, что байты собраны из ожидаемого commit именно разрешённым pipeline; подмена или чужая сборка может выглядеть как обычный образ | ⚠️ **этап 3 реализован и production-проверен 08-31; до закрытия всего контроля остаётся эксплуатационный этап 4.** BuildKit gate из этапа 2 сохраняется. `actions/attest` выпускает для exact build digest keyless SLSA v1 attestation через GitHub OIDC/Sigstore; закреплённый GitHub CLI проверяет trusted root, подпись, subject, signer workflow/digest, source ref/digest и запрет self-hosted runner. Строгий DER-парсер сертификата дополнительно требует Environment `production`, event `push`, runner `github-hosted` и repository ID `1199475908`. Первый run `33384270596` fail-closed остановился до SBOM/deploy при ошибке проверки claims. Исправленный run `33384972241` для commit `ac1d92f…` проверил attestation, прикрепил SBOM и развернул exact `sha256:e727018e…3cd9701`; ревизия `insights-api-00048-dff` получила 100% трафика. Независимый verifier подтвердил identity и Transparency Log. Долгоживущего signing key, нового service account и новых GCP IAM-прав нет. Этап 4 должен закрепить эксплуатационные негативные сценарии и recurring-проверку serving digest |
...
Отдельный provenance-контур реализован до этапа 3 включительно: BuildKit
metadata и keyless GitHub Artifact Attestation exact digest проверяются до
SBOM/deploy. Его эксплуатационные негативные сценарии и recurring-проверка
serving digest остаются этапом 4; это по-прежнему другой контроль, чем SBOM.
