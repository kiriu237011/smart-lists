...
2. **Postgres least privilege и RLS:** архитектурный проект согласован
   2026-08-12. Release-контур этапа 2a подготовлен, но fail-closed выключен
   repository variables; `build:deploy` и фактические права пока прежние.
...
Однако feature flags по умолчанию выключены, Vercel всё ещё выполняет
`build:deploy`, а факт настройки Deployment Check вне репозитория пока не
...
правильность URL. Feature flags отсутствуют, workflow ещё не обращался к БД.
...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Этап 2a подготовлен 2026-08-12, но выключен.** Workflow, target guard и тесты добавлены; `build:deploy` сохраняется до ручного go/no-go по A32. Текущий риск и прикладные контроли пока не изменились |
