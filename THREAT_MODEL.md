...
| A36 | Release workflow входит как `smartlists_migrator`, но создаёт объекты от имени `smartlists_owner` | Иначе login-role снова совмещает credential и ownership либо новые объекты получают неправильного owner | 🟡 **реализация и CI-доказательство готовы 08-13, live apply не выполнен**: чистый PostgreSQL 17 подтвердил `session_user=smartlists_migrator`, `current_user=smartlists_owner`, ownership всех объектов, no-op Prisma и fail-closed отказ при лишнем owner-member; в Neon Environment secrets пока остаётся `neondb_owner` |
...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Этапы 2a и 2b включены и полностью проверены в обеих средах 2026-08-13.** Target guards и no-op миграции прошли для Production и Preview, production promotion дождался migration job того же SHA, Vercel build больше не применяет миграции, а `DIRECT_URL` удалён из обеих сред Vercel. Runtime DML-матрица доказана 213 integration-тестами. Preview и Production прошли автоматические и ручные gates. Для owner/migrator/backup готовы fail-closed configurator и CI-проверка полного staged flow: ownership, no-op Prisma, неизменный runtime ACL, отрицательный membership-тест и data restore. Live cutover ещё не начинался. Следующие шаги — Preview/Production migration scopes, отдельный Production backup scope, scoped context и tenant-RLS |
