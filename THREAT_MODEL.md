...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Release, runtime, operational roles/backup, scoped ordinary tenant data plane и DB-объекты завершены; первый enforcement-canary работает в Preview.** `UserDailyUsage` прошла локальные enable/rollback и fail-closed тесты, PR №107/main CI, live переход `disabled → usage-canary`, пользовательский CRUD smoke и независимый read-only audit. RLS и guard включены только на этой таблице в Preview, FORCE отсутствует; rollback готов именованной операцией. Production и остальные семь tenant-таблиц сохраняют role-wide DML, поэтому staged-переход ещё не завершён. Следующий шаг — период наблюдения и отдельный design/gate следующей связанной группы |
...
|---|---|---|
| 1 | Поэтапно включить tenant-RLS сначала в Preview | Первый `UserDailyUsage` canary включён и проверен; следующая связанная группа требует отдельного design/rollback и live gate, Production — только после полного Preview go/no-go |
...
| Runtime credential имеет table-wide DML там, где RLS ещё не включён | DDL/ownership уже отозваны в обеих средах, но единая runtime-роль технически может читать и менять чужие строки на оставшихся разрешённых таблицах. Основным live-контролем там остаются `listInSpaceWhere` и ownership-проверки приложения. В Preview `UserDailyUsage` уже ограничена RLS и guard; live apply, CRUD smoke и независимый audit прошли. Production и остальные семь tenant-таблиц сохраняют role-wide DML | **Mitigation in progress:** наблюдать первый canary и включать следующие связанные группы только через отдельные Preview gates; Production — после полного go/no-go. Немедленный пересмотр при втором человеке с доступом к Vercel, снятии whitelist или появлении прямого SQL-пути из недоверенного ввода |
