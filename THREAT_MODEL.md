...
2. **Postgres least privilege и RLS:** архитектурный проект согласован
  2026-08-12. Release-контур этапа 2a проверен no-op миграциями Production и
   ...
   release. Cutover-код заменяет `build:deploy` обычной сборкой, однако прежний
   `DIRECT_URL` ещё остаётся в Vercel до успешного Production и Preview release.
   Фактические права runtime, scoped-контекст и RLS ещё не изменены.
   Матрица и gate:
...
| **`DATABASE_URL` / `DIRECT_URL`** | runtime `DATABASE_URL` в Vercel; migration `DIRECT_URL` в GitHub Environments и локальном `.env`, отдельное значение у backup workflow. Прежний Vercel `DIRECT_URL` ожидает удаления после проверенного cutover-релиза | Полный доступ к БД; текущая runtime-роль имеет права DDL |
...
**Разобрано 2026-08-10, риск принят осознанно.** На тот момент очевидная мера — завести runtime-роль без прав DDL — не закрывала сценарий: `build:deploy` применял миграции внутри Vercel под соседним `DIRECT_URL`. Cutover-код от 2026-08-13 убирает эту техническую зависимость, но эффект наступит только после успешного релиза и фактического удаления прежнего секрета из Vercel. До этого роль без DDL у `DATABASE_URL` всё ещё не мешает взять соседнюю переменную.
...
**Release gate доказан, но cutover-код сам по себе ещё не закрывает A31.** Workflow миграции
...
требует `DIRECT_URL`. A31 остаётся открытым до проверенного cutover-релиза и
ручного удаления прежнего секрета из Vercel Production и Preview.
...
| A31 | Владельческая строка подключения недоступна работающему приложению | Пока это не так, любое разделение ролей Postgres декоративно: рядом лежит переменная с DDL | ❌ cutover-код больше не читает `DIRECT_URL` при build/generate, но прежний secret пока физически задан в Vercel. Закрыть строку можно только после успешного Production и Preview release и ручного удаления из обоих environments |
...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Этап 2a, Vercel gate и repository cutover подготовлены 2026-08-13.** Target guards прошли для Production и Preview, миграции были no-op, контрольный production release ждал migration job того же SHA, а Vercel build больше не применяет миграции. До закрытия A31 остаются проверка cutover-релиза и ручное удаление `DIRECT_URL`; текущие DB-права и прикладные контроли пока не изменились |
...
design и repository-часть release cutover; каждый следующий этап меняет статус
отдельно:

| Порядок | Действие | Что меняет |
|---|---|---|
| 1 | Проверить Production и Preview release с обычным build, затем удалить прежний `DIRECT_URL` из обоих Vercel environments и зафиксировать A31 как закрытое | Убирает владельческий секрет из runtime без окна «новый код / старая схема» |
...
| Приложение пока ходит в БД под ролью-владельцем с правами DDL | Риск временно сохраняется: release cutover не меняет фактических прав. Дампы защищены versioning и проверены на восстановимость, PITR Neon покрывает шесть часов | **Mitigation in progress с 2026-08-12:** release migration и repository cutover подготовлены; дальше — удалить прежний Vercel `DIRECT_URL`, ввести runtime least privilege, scoped context и tenant-RLS. Немедленный пересмотр также при втором человеке с доступом к Vercel или снятии whitelist |
