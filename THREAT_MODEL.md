...
| A64 | У каждой новой версии FastAPI-образа до deploy есть машиночитаемая опись именно её финальных байтов | SBOM из checkout/requirements пропустил бы Debian-слой, SBOM по тегу мог бы описать другой образ, а непроверенный JSON был бы описью только по имени | ✅ **реализовано в коде 08-29.** Syft 1.51.0 с проверяемым release checksum читает `${IMAGE}@${DIGEST}` после push и до deploy; gate требует CycloneDX JSON 1.6, непустые components, container metadata и тот же digest. Canonical Version target разрешается через Artifact Registry; детерминированный attachment проверяется по target/type/namespace/files и создаётся fail-closed. `TestCycloneDxSbom` фиксирует контракт. Отдельного архива нет, подписи/provenance нет; live attachment проверяется при первом deploy |
...
| 2 | Syft генерирует CycloneDX JSON 1.6 и прикрепляет его к digest в Artifact Registry | Даёт машиночитаемую опись именно собранного образа. История удалённых образов намеренно не хранится: attachment удаляется вместе с target image | Реализовано в коде 08-29: fail-closed до deploy, digest/format/target закреплены тестом. Критерий завершения — успешный production run и чтение attachment обратно из Artifact Registry |
