...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Release, runtime и operational owner/migrator/backup этапы завершены 2026-08-13; обычный tenant data plane переведён на scoped context 2026-08-14.** Foundation, `spaces`, server read-path, user quota, space mutations, DB-фазы AI insights/attachments, ListGroup lifecycle/membership, List/sharing/note/item lifecycle и item movement используют scoped API. `moveItem`/`moveItemToList` сохраняют проверку обоих списков, соседей и лимита, атомарные rebalance/move/copy и каскад подпунктов; realtime recipients собираются до commit. Глобальный Prisma import из `src/app/actions/index.ts` удалён, прямой allowlist сократился с 13 до 5, per-action guard защищает 22 Actions. DB-тесты доказывают ownership/share-space semantics, конкурентную атомарность, cross-space отказ и commit до внешних эффектов; S3/AI/Pusher не удерживают DB-транзакции и `after()` не читает tenant-таблицы. Для глобальной attachment-квоты и cross-space stale-cleanup до enforcement нужен узкий DB-helper — обычная space-policy изменила бы семантику. RLS выключен, поэтому фактическая DB-изоляция строк пока не завершена. Следующие шаги — helper, policies без enforcement, отрицательные Alice/Bob тесты и поэтапное включение tenant-RLS |
...
|---|---|---|
| 1 | Подготовить узкий attachment helper и tenant-RLS policies без немедленного enforcement | Обычный tenant data plane уже scoped; реальный DB-контроль появится только после проверки и включения политик |
