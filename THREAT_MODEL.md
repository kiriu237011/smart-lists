...
| 1.5 | Починить release-job production-миграций: она перестала запускаться | Mitigate | 🔴 **2026-09-06: исправление подготовлено локально; production-подтверждение остаётся открытым.** Граница регрессии установлена по соседним push-прогонам main: [33127721197](https://github.com/kzhirikhin/smart-lists/actions/runs/33127721197) на `a0e477d` успешно выполнил `Apply production migrations`; [33133159737](https://github.com/kzhirikhin/smart-lists/actions/runs/33133159737) на `6768430` уже пропустил всю job. Сегодняшний [34019725241](https://github.com/kzhirikhin/smart-lists/actions/runs/34019725241) подтверждает тот же дефект: шесть прямых needs успешны, migration skipped без шагов. Коммит `f2bac03` добавил `gate` в needs миграций; gate зависит от штатно пропускаемых на push `docs` и `dependency-review`. У gate есть `always()`, у migration нет status-функции, поэтому действует неявный `success()` и skip распространяется по цепочке зависимостей. Это соответствует [документированной семантике GitHub](https://docs.github.com/en/actions/how-tos/write-workflows/choose-what-workflows-do/use-jobs) и объясняет наблюдаемую границу. Прежняя гипотеза о пустом Environment, скрывающем repository vars, не поддерживается [документацией переменных](https://docs.github.com/en/actions/reference/workflows-and-actions/variables); условие флага и environment в регрессионном diff не менялись. Перед публикацией исправления аутентифицированный API подтвердил `ENABLE_PRODUCTION_MIGRATION=true`, без изменений с 2026-08-12; первоначальный 401 был ограничением доступа из sandbox. Отдельного экспериментального workflow не запускали. В локальном workflow добавлен явный `!cancelled()` и проверка `success` каждого из шести прямых needs. Статический regression-тест берёт набор needs из workflow и требует точную конъюнкцию guards: добавленный needs без проверки, возврат неявного success() или ослабление условия ломают тест. Это контроль формы YAML, а не эмуляция планировщика. Реальный post-merge прогон и удержание Vercel alias того же SHA ещё нужно подтвердить; пункт не закрыт. Последнее изменение prisma/migrations в текущем checkout — 2026-08-26, поэтому новых неприменённых миграций из последующих коммитов не обнаружено; live-БД и Vercel gate в этой проверке не проверялись. Риск рассинхронизации будущей схемы остаётся открытым; затрагивает A32 и DATABASE_SECURITY.md |
