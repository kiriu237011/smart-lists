...
2. **Postgres least privilege и RLS:** архитектурный проект согласован
  2026-08-12. Release-контур этапа 2a проверен no-op миграциями Production и
   ...
   из Vercel Production и Preview. Следующий открытый шаг — runtime без DDL;
   scoped-контекст и RLS также ещё не внедрены.
   Матрица и gate:
...
| A31 | Владельческая строка подключения недоступна работающему приложению | Иначе любое разделение ролей Postgres декоративно: рядом лежит переменная с DDL | ✅ **закрыто 08-13** после проверенного cutover PR №64: `DIRECT_URL` удалён из Vercel Production и Preview, повторный `vercel env ls` показывает только runtime `DATABASE_URL`; migration credentials остались в отдельных GitHub Environments/backup workflow |
...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Этап 2a завершён; этап 2b подготовлен и проаудирован 2026-08-13.** Target guards и no-op миграции прошли для Production и Preview, production promotion дождался migration job того же SHA, Vercel build больше не применяет миграции, а `DIRECT_URL` удалён из обеих сред Vercel. Точная runtime DML-матрица и fail-closed role tooling проверены на Docker PostgreSQL: 213 integration-тестов прошли под ролью без DDL/BYPASS/ownership. Read-only audit `dev` и `production` подтвердил отдельные endpoints, ожидаемую схему, отсутствие RLS и единственную владельческую роль `neondb_owner` с `neon_superuser`/BYPASS. Preview/Production credential ещё не переключались, поэтому фактические runtime-права пока не изменились |
...
|---|---|---|
| 1 | Создать owner/migrator/runtime-роли и заменить Vercel `DATABASE_URL` на credential без DDL после проверки прав и отката | Ограничивает ущерб от runtime-компрометации теперь, когда соседний владельческий secret удалён |
...
| Приложение пока ходит в БД под ролью-владельцем с правами DDL | Риск временно сохраняется: release cutover убрал соседний owner credential, но ещё не сузил сам `DATABASE_URL`. Дампы защищены versioning и проверены на восстановимость, PITR Neon покрывает шесть часов | **Mitigation in progress с 2026-08-12:** release migration и cutover завершены; runtime-матрица, rollback и restricted-role integration gate подготовлены и доказаны локально 2026-08-13. Облачный read-only audit завершён и подтвердил риск; следующий отдельный шаг — Preview cutover, затем Production go/no-go, scoped context и tenant-RLS. Немедленный пересмотр также при втором человеке с доступом к Vercel или снятии whitelist |
