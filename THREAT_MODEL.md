...
**Последняя сверка с кодом:** 2026-08-13 (полная); 2026-08-21 — точечно: путь
`prisma → @prisma/config → deepmerge-ts` и scope алерта Dependabot
...
- полный deployment-time разбор не выполнен: границы GitHub Actions рассмотрены
  для бэкапов, автоматической синхронизации Preview OAuth proxy и пути образа
...
2. **Postgres least privilege и RLS:** архитектурный проект согласован
  2026-08-12. Release-контур этапа 2a проверен no-op миграциями Production и
   ...
   произошёл после migration success того же SHA. Backup scope применён в
   Production, repository secret переведён на `smartlists_backup` — см. A37.
   Scoped-контекст и RLS не внедрены.
   Матрица и gate:
...
| 2.5 | Staged-переход Postgres: release migration, runtime least privilege, scoped context, tenant-RLS | **Mitigate** | 🟡 **Release, runtime и operational owner/migrator/backup этапы завершены 2026-08-13; foundation, `spaces`, server read-path, user quota, space mutations, DB-фазы AI insights/attachments, ListGroup lifecycle/membership, List lifecycle, sharing lifecycle, note mutations и item lifecycle реализованы 2026-08-14.** Прямой allowlist сократился с 13 до 6; частично перенесённый `index.ts` дополнительно защищён per-action guard для двадцати group/list/sharing/note/item actions. Sharing больше не создаёт default-space от имени получателя: migration/Auth.js гарантируют строку, а составной FK даёт fail-closed rollback без privileged helper; чужой listId также не раскрывает существование email. Note Actions сохраняют условную запись по `noteVersion`, включая реальную конкурентную гонку, и fail-closed привязку к пространству. Item lifecycle сохраняет editor-права, лимиты, одноуровневую вложенность, каскады и атомарный parent cache в scoped-транзакции. DB-тесты доказывают ownership/share-space semantics, конкурентную атомарность, atomic rebalance, cross-space отказ и commit до внешних эффектов. S3/AI/Pusher не удерживают DB-транзакции; attachment/list/sharing/note/item lifecycle передают заранее собранных получателей в `after()` без tenant-чтения. Для глобальной attachment-квоты и cross-space stale-cleanup до enforcement нужен узкий DB-helper — обычная space-policy изменила бы семантику. Остальные tenant-мутации и специальные потоки ещё не перенесены, RLS выключен, поэтому фактическая DB-изоляция строк пока не завершена. Следующие шаги — перенос оставшихся потоков, helper/policies и поэтапное включение tenant-RLS |
...
|---|---|---|
| 1 | Перенести tenant-потребителей на проверенный scoped transaction context; затем подготовить tenant-RLS policies без немедленного enforcement | Foundation API и его DB-тесты готовы; реальный контроль появится только после переноса потоков и включения политик |
...
| Runtime credential до RLS имеет table-wide DML в разрешённых таблицах | DDL/ownership уже отозваны в обеих средах, но единая runtime-роль технически может читать и менять чужие строки там, где DML разрешён. Основным контролем остаются `listInSpaceWhere` и ownership-проверки приложения. Scoped API уже покрывает space helpers, server read-path, user quota, space mutations, DB-фазы AI insights/attachments, ListGroup lifecycle/membership, List lifecycle, sharing lifecycle, note mutations и item lifecycle, но без RLS и полного переноса tenant-потоков ещё не даёт целостной DB-изоляции. Attachment enforcement дополнительно требует узкий helper для глобальной пользовательской квоты и stale-cleanup между пространствами | **Mitigation in progress:** перенос всех tenant-потоков на scoped transaction context, helper/policies без enforcement, отрицательные Alice/Bob тесты и поэтапное tenant-RLS. Немедленный пересмотр при втором человеке с доступом к Vercel, снятии whitelist или появлении прямого SQL-пути из недоверенного ввода |
