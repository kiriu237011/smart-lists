...
основной server read-path и user quota локально проверены, RLS выключен
**Дата:** 2026-08-14
...
запросов проходят ровно два. Allowlist прямых импортов глобального Prisma
сократился с 13 до 9; новые обходы запрещены. Пока остальные tenant-потоки не
...
4. **Scoped Prisma API — в работе.** Foundation `withUserDb`/`withSpaceDb`,
   space helpers, основной server read-path и user quota реализованы и локально
   проверены; далее перенести tenant-мутации и специальные потоки, сохранив
   поведение и тесты.
