...
до 7; новые обходы запрещены. Пока остальные tenant-потоки не переведены и RLS
...
4. **Scoped Prisma API — в работе.** Foundation `withUserDb`/`withSpaceDb`,
   ...
   фазы AI insights реализованы и локально проверены; далее перенести остальные
