...
**Статус:** этапы 2a–2c завершены; foundation scoped Prisma API реализован и
локально проверен, перенос tenant-потоков ещё не начат, RLS выключен
**Дата:** 2026-08-14
...
**Foundation 2026-08-14:** `src/lib/scoped-db.ts` реализует оба wrapper без
подключения production-потребителей. Контекст устанавливается
...
контекста между транзакциями. Статический тест фиксирует текущие 13 прямых
импортов глобального Prisma как переходный allowlist: новые обходы запрещены,
а список должен сокращаться по мере переноса потоков. Пока ни один tenant-
потребитель не переведён и RLS выключен, этот foundation сам по себе не меняет
изоляцию данных и не считается новым security-контролем.
...
4. **Scoped Prisma API — в работе.** Foundation `withUserDb`/`withSpaceDb`
   реализован и локально проверен; далее перенести все tenant-запросы и
   специальные потоки, сохранив поведение и тесты.
