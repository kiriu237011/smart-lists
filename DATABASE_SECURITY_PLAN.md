...
**Статус:** этапы 2a–2c завершены; foundation scoped Prisma API и первая
consumer-группа (`spaces`) локально проверены, RLS выключен
**Дата:** 2026-08-14
...
чужого/некорректного пространства. Переходный allowlist прямых импортов
глобального Prisma сократился с 13 до 12; новые обходы запрещены. Пока остальные
tenant-потоки не переведены и RLS выключен, это ещё не завершённый контроль
изоляции строк.
