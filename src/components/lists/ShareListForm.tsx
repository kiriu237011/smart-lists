 ...
 *   - `ShareListButton` — кнопка-триггер (управляется снаружи через `isOpen`/`onToggle`).
 *   - `ShareListForm`   — панель с бейджами участников и формой приглашения.
 *
...
type ShareListButtonProps = {
  isOpen: boolean;
  onToggle: () => void;
  /** Количество пользователей с доступом — для бейджа-счётчика. */
  sharedCount: number;
};

/** Кнопка-пилюля для раскрытия формы совместного доступа. */
...
/**
 * Панель управления совместным доступом: бейджи участников и форма приглашения.
...
export default function ShareListForm({
  ...
  const handleSubmit = async (event: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    const tempUser = { id: tempId, email: normalizedEmail, name: null };
