...
- Ответ FastAPI проверяется Zod-схемой после JSON-разбора: `insight` обязан быть
  ...
  риск принят для приватного IAM-защищённого сервиса, где генерация Anthropic
  дополнительно ограничена `max_tokens=2048`.
...
- AI: `INSIGHTS_SERVICE_URL` — только в Vercel Production. В локальном `.env` и
  ...
  убраны 2026-09-02. Состав переменных Vercel этой сверкой не покрыт: он не
  читается ни дашбордом, ни API и проверяется только `vercel env ls` вручную;
